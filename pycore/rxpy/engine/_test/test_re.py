...
#        # self._re.sub(x, y, u'') should return u'', not '', and
#        # self._re.sub(x, y, '') should return '', not u''.
#        # Also:
#        # self._re.sub(x, y, unicode(x)) should return unicode(y), and
#        # self._re.sub(x, y, str(x)) should return
#        #     str(y) if isinstance(y, str) else unicode(y).
#        for x in 'x', u'x':
#            for y in 'y', u'y':
#                z = self._re.sub(x, y, u'')
...
#                #
#                z = self._re.sub(x, y, unicode(x))
#                self.assertEqual(z, y)
#                self.assertEqual(type(z), unicode)
#                #
#                z = self._re.sub(x, y, str(x))
#                self.assertEqual(z, y)
...
#                                   "abc bcd bc abxd", self._re.LOCALE).group(1), "bx")
        ...
        self.assertEqual(self._re.search(r"^abc$", u"\nabc\n", self._re.M).group(0), "abc")
        self.assertEqual(self._re.search(r"^\Aabc\Z$", u"abc", self._re.M).group(0), "abc")
        self.assertEqual(self._re.search(r"^\Aabc\Z$", u"\nabc\n", self._re.M), None)
...
#                                   "1aa! a", self._re.LOCALE).group(0), "1aa! a")
    ...
    def test_ignore_case(self):
        ...
        self.assertEqual(self._re.match("abc", u"ABC", self._re.I).group(0), "ABC")
...
#        self.assertEqual(self._re.L, self._re.LOCALE)
        ...
        self.assertEqual(self._re.S, self._re.DOTALL)
...
#        self.assertNotEqual(scanner.scanner.scanner("").pattern, None)
    ...
    def test_bug_926075(self):
        ...
        self.assertTrue(self._re.compile('bug_926075') is not
                     self._re.compile(eval("u'bug_926075'")))
    ...
    def test_bug_6561(self):
        ...
        for x in decimal_digits:
            self.assertEqual(self._re.match('^\d$', x, self._re.UNICODE).group(0), x)
        ...
        for x in not_decimal_digits:
            assert self._re.match('^\d$', x, self._re.UNICODE) is None
    ...
    def test_empty_array(self):
        ...
        for typecode in 'cbBuhHiIlLfd':
            ...
            self.assertEqual(self._re.compile("bla").match(a), None)
            self.assertEqual(self._re.compile("").match(a).groups(), ())
    ...
    def test_all(self):
        from rxpy.engine._test.re_tests import tests, SUCCEED, FAIL, SYNTAX_ERROR
    ...
    #                    continue # unicode test case
                    ...
                    # still succeeds.
                    obj=self._re.compile(unicode(pattern, "latin-1"))
                    result = obj.search(s)
                    if result is None:
